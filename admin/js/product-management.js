...
const fetchProductList = async () => {
  ...
  quantityTitle.textContent = `Tada! We have ${productList.length} products!`;
...
// Working at the fields on working overlay product-management.html
const handleValidate = (element, validateWarningName, typeOfChecking) => {
  const checkIsEmptyString = /[^\s]/;
  const checkIsPositiveNumber = /^[+]?([0-9]+(?:[\.][0-9]*)?|\.[0-9]+)$/;
  const checkIsPositiveWholeNumber = /^[+]?[0-9]+$/;

  switch (typeOfChecking) {
    case 'checkIsPositiveWholeNumber':
      validateData(
        element.value,
        validateWarningName,
        checkIsPositiveWholeNumber
      );
      break;
    case 'checkIsPositiveNumber':
      validateData(element.value, validateWarningName, checkIsPositiveNumber);
      break;
    default:
      validateData(element.value, validateWarningName, checkIsEmptyString);
  }
};
const validateData = (value, validateWarningName = '', regex) => {
  const validateWarning = document.querySelector(
    `.validate-warning--${validateWarningName}`
  );

  // check empty string is still not work. must fix
  const result = regex.test(value);

  result
    ? (validateWarning.style.display = 'none')
    : (validateWarning.style.display = 'block');
...
const pushNewProductToLocalStorage = (newProduct = {}) => {
  ...
  quantityTitle.textContent = `Tada! We have ${newMaSP} products!`;
...
const updateProduct = () => {
  ...
  const processingUpdating = () => {
    ...
    // reset upload image input
    // fix the error after upload an image
    // if we upload the same image,
    // onchang event will not be invoked to change displaying image on working overlay
    inProductImg.value = null;
    imgProductName.textContent = 'No file upload!';
